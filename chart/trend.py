...
def swing_zz(chart, ticks, color='blue', span=6, backtrace=3, debug=0):
    from indicators.trend import swing_zz as _swing_zz

    fig, ax = chart

    if debug == 2:
        zigzags, lows, highs = _swing_zz(ticks, span=span, backtrace=backtrace,
                                         debug=2)
    else:
        zigzags,  = _swing_zz(ticks, span=span, backtrace=backtrace)
